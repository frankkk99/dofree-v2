...
const storagePrefix = 'dofree:all-movies-sections';
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-50 border-b border-white/5 bg-black/65 backdrop-blur-2xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-3 px-3 md:h-20 md:px-8">
        <div><p className="text-lg font-black tracking-tight text-red-500 md:text-2xl">DOFree By Frank</p><p className="hidden text-xs text-white/40 sm:block">รวมหนังและซีรีส์ พร้อมข้อมูลไทย / อังกฤษ</p></div>
        <nav className="hidden items-center gap-6 text-sm font-medium text-white/60 md:flex"><a href="#movies" className="hover:text-white">หนัง</a><a href="#categories" className="hover:text-white">หมวดหมู่</a></nav>
    ...
    <section className="relative min-h-[620px] overflow-hidden pt-20 md:min-h-[760px]">
      ...
      <div className="relative z-10 mx-auto flex min-h-[520px] max-w-7xl items-center px-4 md:min-h-[650px] md:px-8">
        <div className="max-w-2xl pt-12 md:pt-20">
          ...
          <div className="mt-5 flex flex-wrap gap-3 text-xs font-semibold text-white/65"><span className="text-yellow-300">★ {movie.rating}</span><span>{movie.year}</span><span>{thaiGenre(movie)}</span></div>
          <div className="mt-8 flex flex-wrap gap-3"><button onClick={() => onPlay(movie)} className="rounded-xl bg-white px-6 py-3 text-sm font-black text-black hover:bg-white/85">▶ รับชมตัวอย่าง</button><button onClick={() => onInfo(movie)} className="rounded-xl border border-white/15 bg-white/15 px-6 py-3 text-sm font-black text-white hover:bg-white/25">ⓘ ข้อมูลเพิ่มเติม</button></div>
        </div>
      </div>
    </section>
  );
    ...
    <section id="categories" className="relative z-30 mx-auto -mt-24 max-w-7xl px-3 md:px-8">
      <div className="rounded-3xl border border-white/10 bg-black/50 p-3 shadow-2xl backdrop-blur-2xl">
        <div className="grid gap-3 md:grid-cols-[320px_1fr]">
          <label className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/10 px-3 py-3 text-xs text-white/60"><span>⌕</span><input value={query} onChange={(event) => onQuery(event.target.value)} placeholder="ค้นหาไทย / English" className="w-full bg-transparent font-semibold text-white outline-none placeholder:text-white/45" /></label>
          <div className="no-scrollbar flex gap-2 overflow-x-auto rounded-xl bg-white/[0.04] p-2">{genreOptions.map((genre) => <button key={genre} onClick={() => onGenre(genre)} className={`shrink-0 rounded-full px-3 py-1.5 text-[11px] font-bold transition ${activeGenre === genre ? 'bg-red-600 text-white' : 'bg-white/10 text-white/70 hover:bg-white/20'}`}>{genre}</button>)}</div>
        </div>
      </div>
    </section>
  );
...
function BilingualTitle({ movie }: { movie: Movie }) {
  return <div className="space-y-1"><div className="flex items-start gap-1.5"><span className="mt-0.5 rounded bg-white/10 px-1.5 py-0.5 text-[8px] font-black text-white/45">EN</span><h3 className="line-clamp-1 text-xs font-black text-white md:text-sm">{movie.title}</h3></div><div className="flex items-start gap-1.5"><span className="mt-0.5 rounded bg-red-500/20 px-1.5 py-0.5 text-[8px] font-black text-red-100/70">TH</span><p className="line-clamp-1 text-[11px] font-semibold text-red-100/65">{movie.thaiTitle || 'ยังไม่มีชื่อไทย'}</p></div></div>;
}
...
function MovieCard({ movie, onSelect, onFavorite, isFavorite, compact = false }: { movie: Movie; onSelect: (movie: Movie) => void; onFavorite: (movie: Movie) => void; isFavorite: boolean; compact?: boolean }) {
  return (
    <button type="button" onClick={() => onSelect(movie)} className="group min-w-0 text-left transition duration-300 hover:-translate-y-1">
      <div className="relative overflow-hidden rounded-xl bg-white/5 shadow-2xl ring-1 ring-white/10 transition group-hover:ring-red-500/50"><img src={movie.poster} alt={movie.title} loading="lazy" decoding="async" className="aspect-[2/3] w-full object-cover transition duration-500 group-hover:scale-105" /><div className="absolute left-2 top-2 rounded-full bg-black/75 px-2 py-1 text-[10px] font-black text-yellow-300">★ {movie.rating}</div><span onClick={(event) => { event.stopPropagation(); onFavorite(movie); }} className={`absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full text-xs ${isFavorite ? 'bg-red-600 text-white' : 'bg-black/65 text-white/70'}`}>♥</span></div>
...
function AutoNowPlayingRow({ row, favorites, onSelect, onFavorite, onShowAll }: { row: MovieRowData; favorites: string[]; onSelect: (movie: Movie) => void; onFavorite: (movie: Movie) => void; onShowAll: (row: MovieRowData) => void }) {
  ...
  return (
    <section id="movies" className="mx-auto max-w-7xl px-3 py-8 md:px-8">
      <style>{`@keyframes dofree-now-playing-marquee{0%{transform:translate3d(0,0,0)}100%{transform:translate3d(-50%,0,0)}}.dofree-now-playing-rail{-webkit-overflow-scrolling:touch;scroll-behavior:smooth}.dofree-now-playing-track{width:max-content;animation:dofree-now-playing-marquee 70s linear infinite;will-change:transform}.dofree-now-playing-track.is-paused{animation-play-state:paused}@media(prefers-reduced-motion:reduce){.dofree-now-playing-track{animation:none}}`}</style>
      <div className="mb-4 flex items-end justify-between gap-4"><div><div className="inline-flex rounded-full border border-red-400/25 bg-red-500/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.24em] text-red-200">กำลังฉาย</div><h2 className="mt-2 text-xl font-black text-white md:text-3xl">{row.title}</h2><p className="mt-1 text-sm text-white/45">{row.subtitle} • {row.movies.length} เรื่อง</p></div><button onClick={() => onShowAll(row)} className="rounded-full bg-white/10 px-3 py-1.5 text-[11px] font-bold text-white/65 hover:bg-white/20 hover:text-white">ทั้งหมด</button></div>
      <div onPointerDown={() => setPaused(true)} onPointerUp={() => window.setTimeout(() => setPaused(false), 1300)} onPointerCancel={() => setPaused(false)} onTouchStart={() => setPaused(true)} onTouchEnd={() => window.setTimeout(() => setPaused(false), 1300)} className="dofree-now-playing-rail no-scrollbar overflow-x-auto overscroll-x-contain pb-3"><div className={`dofree-now-playing-track flex gap-3 ${paused ? 'is-paused' : ''}`}>{marqueeItems.map((movie, index) => <div key={`now-playing-${index}-${keyOf(movie)}`} className="w-[43vw] shrink-0 sm:w-[28vw] md:w-[190px] lg:w-[205px]"><MovieCard movie={movie} onSelect={onSelect} onFavorite={onFavorite} isFavorite={favorites.includes(keyOf(movie))} /></div>)}</div></div>
    </section>
  );
}

function MovieRow({ row, rowKey, visibleRows, columns, noMore, loadingMore, favorites, onSelect, onFavorite, onToggleRows, searchMode = false }: { row: MovieRowData; rowKey: string; visibleRows: number; columns: number; noMore: boolean; loadingMore: boolean; favorites: string[]; onSelect: (movie: Movie) => void; onFavorite: (movie: Movie) => void; onToggleRows: (row: MovieRowData, rowKey: string, expandedToEnd: boolean) => void; searchMode?: boolean }) {
  ...
  const canExpand = row.movies.length > initialCount || Boolean(row.slug && !noMore);
  const expandedToEnd = visibleCount >= row.movies.length && (noMore || !row.slug) && visibleRows > 3;
  return (
    <section id="movies" className="mx-auto max-w-7xl px-3 py-7 md:px-8">
      <div className="mb-4 flex items-end justify-between gap-4"><div><h2 className="text-lg font-black text-white md:text-2xl">{row.title}</h2><p className="mt-1 text-sm text-white/45">{row.subtitle} • แสดง {preview.length} จาก {noMore ? row.movies.length : `${row.movies.length}+`} เรื่อง</p></div></div>
      ...
      {canExpand ? <div className="mt-5 flex justify-center"><button disabled={loadingMore} onClick={() => onToggleRows(row, rowKey, expandedToEnd)} className="rounded-full border border-white/10 bg-white/10 px-5 py-2.5 text-xs font-black text-white/70 hover:border-red-400/40 hover:bg-red-600/25 hover:text-white disabled:cursor-wait disabled:opacity-50">{loadingMore ? 'กำลังโหลด...' : expandedToEnd ? 'ยุบ' : 'ดูเพิ่มเติม'}</button></div> : null}
    </section>
  );
}
...
export function ExperienceShellAllMoviesRows() {
  ...
  const [nextPageBySlug, setNextPageBySlug] = useState<Record<string, number>>({});
  const [noMoreBySlug, setNoMoreBySlug] = useState<Record<string, boolean>>({});
  const [loadingMoreBySlug, setLoadingMoreBySlug] = useState<Record<string, boolean>>({});
  ...
  useEffect(() => {
    ...
    async function loadHome() {
      try {
        ...
        setNextPageBySlug(Object.fromEntries(data.rows.filter((row) => row.slug).map((row) => [row.slug, (row.loadedPages || 3) + 1])));
      } catch {
        if (!mounted) return;
  ...
  useEffect(() => { setVisibleRowsByKey({}); }, [query, activeGenre]);
  ...
  const toggleFavorite = (movie: Movie) => {
    ...
    setFavorites((current) => { const next = current.includes(id) ? current.filter((item) => item !== id) : [...current, id]; localStorage.setItem(`${storagePrefix}:favorites`, JSON.stringify(next)); return next; });
  };

  const appendMoviesToRow = (slug: string, movies: Movie[]) => {
    if (!movies.length) return;
    setRows((current) => current.map((row) => row.slug === slug ? { ...row, movies: uniqueMovies([...row.movies, ...movies]) } : row));
    setAllMovies((current) => uniqueMovies([...current, ...movies]));
  };

  const toggleRows = async (row: MovieRowData, rowKey: string, expandedToEnd: boolean) => {
    if (expandedToEnd) {
      setVisibleRowsByKey((current) => ({ ...current, [rowKey]: 3 }));
      return;
    }

    const visibleRows = visibleRowsByKey[rowKey] || 3;
    const visibleCount = visibleRows * columns;
    const hasLoadedHiddenMovies = visibleCount < row.movies.length;

    if (hasLoadedHiddenMovies || !row.slug || noMoreBySlug[row.slug]) {
      setVisibleRowsByKey((current) => ({ ...current, [rowKey]: visibleRows + 3 }));
      return;
    }

    const slug = row.slug;
    const nextPage = nextPageBySlug[slug] || 4;
    setLoadingMoreBySlug((current) => ({ ...current, [slug]: true }));
    try {
      const response = await fetch(`/api/tmdb/category?slug=${encodeURIComponent(slug)}&page=${nextPage}&pages=3`, { cache: 'no-store' });
      const data = (await response.json()) as CategoryPageResponse;
      if (!response.ok) throw new Error('load failed');
      appendMoviesToRow(slug, data.movies || []);
      setNextPageBySlug((current) => ({ ...current, [slug]: data.nextPage || nextPage + 3 }));
      setNoMoreBySlug((current) => ({ ...current, [slug]: !data.hasMore || !(data.movies || []).length }));
      setVisibleRowsByKey((current) => ({ ...current, [rowKey]: visibleRows + 3 }));
    } catch {
      setNoMoreBySlug((current) => ({ ...current, [slug]: true }));
    } finally {
      setLoadingMoreBySlug((current) => ({ ...current, [slug]: false }));
    }
  };

  const selectMovie = async (movie: Movie) => { setSelectedMovie(movie); setDetailLoading(true); const detailed = await fetchDetails(movie).catch(() => movie); setSelectedMovie(detailed); setDetailLoading(false); };
  const playMovie = async (movie: Movie) => { const detailed = movie.trailerKey ? movie : await fetchDetails(movie).catch(() => movie); setPlayerMovie(detailed); };
  const login = () => { const name = 'Frank'; localStorage.setItem(`${storagePrefix}:user`, name); setUserName(name); setLoginOpen(false); };
    ...
    <main className="min-h-screen bg-black text-white">
      ...
      <div className="pb-16 pt-8">
        {filteredRows.map((row, index) => {
          const rowKey = `${row.slug || row.title}-${index}`;
          if (!searchMode && isAll(activeGenre) && rowIsNowPlaying(row)) return <AutoNowPlayingRow key={rowKey} row={row} favorites={favorites} onSelect={selectMovie} onFavorite={toggleFavorite} onShowAll={setCategoryRow} />;
          const slug = row.slug || rowKey;
          return <MovieRow key={rowKey} rowKey={rowKey} row={row} visibleRows={visibleRowsByKey[rowKey] || 3} columns={columns} noMore={Boolean(noMoreBySlug[slug])} loadingMore={Boolean(loadingMoreBySlug[slug])} favorites={favorites} onSelect={selectMovie} onFavorite={toggleFavorite} onToggleRows={toggleRows} searchMode={searchMode} />;
        })}
      </div>
